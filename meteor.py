...
countries = countries = [geolocator.reverse("{}, {}".format(item['reclat'], item['reclong'])).address.split(', ') for item in result]
...
def get_article_data(country_name, year):
    ...
    return requests.get(base_url + path).json()[-1][0]
#----------------------------------->>>
